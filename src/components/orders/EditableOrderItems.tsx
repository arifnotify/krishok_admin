...
import {adminEditOrder} from "@/src/services/order.service";
...
export default function EditableOrderItems({orderId,items,setItems,locked}:Props){
...
if(exist){
setItems(items.map(i=>i.product===p._id?{...i,quantity:i.quantity+1,totalPrice:i.price*(i.quantity+1)}:i));
}else{
setItems([...items,{product:p._id,productName:getName(p),productImage:p.images?.[0]||"",price,quantity:1,totalPrice:price}]);
}
...
if(!permission("Change product quantity?"))return;
...
data[index]={...data[index],quantity:qty,totalPrice:data[index].price*qty};
...
const filtered=products.filter(product=>getName(product).toLowerCase().includes(search.toLowerCase()));
return(
...
<button disabled={locked} onClick={()=>{setShowProducts(true);loadProducts()}} className="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
<Plus size={16}/>
Add Product
</button>

<button disabled={saving||locked} onClick={saveChanges} className="bg-blue-600 text-white px-4 py-2 rounded-lg">
